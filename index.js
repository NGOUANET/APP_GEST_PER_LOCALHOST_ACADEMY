  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création du matériel" });
      res
        .status(201)
        .json({ message: "Matériel créé avec succès", id: result.insertId });
    }
  );
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la mise à jour du matériel" });
...
function deleteMateriel(req, res) {
  ...
  db.query("DELETE FROM materiels WHERE id = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la suppression du matériel" });
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Matériel non trouvé" });
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création de la salle" });
      res
        .status(201)
        .json({ message: "Salle créée avec succès", id: result.insertId });
    }
  );
}
...
function createFormation(req, res) {
  const {
    ...
    Programme_de_cours,
    Duree,
    A_un_stage_Professionel,
    Duree_stage_Professionnel,
    id_departement,
    date,
  } = req.body;
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res
        .status(201)
        .json({ message: "Formation créée avec succès", id: result.insertId });
    }
  );
}
...
function createDepartement(req, res) {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res
        .status(201)
        .json({ message: "Département créé avec succès", id: result.insertId });
    }
  );
}
...
function getDepartements(req, res) {
  db.query("SELECT * FROM departements", (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des départements" });
    res.json(results);
  });
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la mise à jour du département" });
      if (result.affectedRows === 0)
...
function deleteDepartement(req, res) {
  ...
  db.query("DELETE FROM departements WHERE id = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la suppression du département" });
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Département non trouvé" });
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création du contrat" });
      res
        .status(201)
        .json({ message: "Contrat créé avec succès", id: result.insertId });
    }
  );
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création de l'entrée" });
      res
        .status(201)
        .json({ message: "Entrée créée avec succès", id: result.insertId });
    }
  );
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création de la sortie" });
      res
        .status(201)
        .json({ message: "Sortie créée avec succès", id: result.insertId });
    }
  );
}
...
function getSorties(req, res) {
  db.query("SELECT * FROM sorties", (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des sorties" });
    res.json(results);
  });
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la mise à jour de la sortie" });
      if (result.affectedRows === 0)
...
function deleteSortie(req, res) {
  ...
  db.query("DELETE FROM sorties WHERE id = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la suppression de la sortie" });
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Sortie non trouvée" });
...
function createEtudiant(req, res) {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res
        .status(201)
        .json({ message: "Étudiant créé avec succès", id: result.insertId });
    }
  );
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création de la paie" });
      res
        .status(201)
        .json({ message: "Paie créée avec succès", id: result.insertId });
    }
  );
}
...
function getPaies(req, res) {
  db.query("SELECT * FROM paies", (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des paies" });
    res.json(results);
  });
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la mise à jour de la paie" });
      if (result.affectedRows === 0)
        return res.status(404).json({ error: "Paie non trouvée" });
...
function deletePaie(req, res) {
  ...
  db.query("DELETE FROM paies WHERE id = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la suppression de la paie" });
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Paie non trouvée" });
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création du planning" });
      res
        .status(201)
        .json({ message: "Planning créé avec succès", id: result.insertId });
    }
  );
}
...
function getPlannings(req, res) {
  db.query("SELECT * FROM planning", (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des plannings" });
    res.json(results);
  });
}
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la mise à jour du planning" });
      if (result.affectedRows === 0)
...
function deletePlanning(req, res) {
  ...
  db.query("DELETE FROM planning WHERE id = ?", [id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Erreur lors de la suppression du planning" });
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Planning non trouvé" });
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Erreur lors de la création de l'association" });
      res
        .status(201)
        .json({
...
function deleteFormationEtudiant(req, res) {
  ...
  db.query(
    "DELETE FROM formations_etudiant WHERE id = ?",
    [id],
    (err, result) => {
      if (err)
        return res
    ...
    (err, result) => {
      if (err)
        return res
          ...
          .json({
            error:
              "Erreur lors de la création de l'entrée de matériel utilisateur",
          });
      res
        .status(201)
        .json({
          message: "Entrée de matériel utilisateur créée avec succès",
          id: result.insertId,
        });
    }
  );
  ...
  db.query("SELECT * FROM materiel_utilisateur", (err, results) => {
    if (err)
      return res
        .status(500)
        .json({
          error: "Erreur lors de la récupération des matériels utilisateurs",
        });
    res.json(results);
  });
}
    ...
    (err, result) => {
      if (err)
        return res
          ...
          .json({
            error:
              "Erreur lors de la mise à jour de l'entrée de matériel utilisateur",
          });
      if (result.affectedRows === 0)
        return res
          .status(404)
          .json({ error: "Entrée de matériel utilisateur non trouvée" });
      res.json({
        message: "Entrée de matériel utilisateur mise à jour avec succès",
      });
    }
  );
}
...
function deleteMaterielUtilisateur(req, res) {
  ...
  db.query(
    "DELETE FROM materiel_utilisateur WHERE id = ?",
    [id],
    (err, result) => {
      if (err)
        return res
          ...
          .json({
            error:
              "Erreur lors de la suppression de l'entrée de matériel utilisateur",
          });
      if (result.affectedRows === 0)
        return res
          .status(404)
          .json({ error: "Entrée de matériel utilisateur non trouvée" });
      res.json({
        message: "Entrée de matériel utilisateur supprimée avec succès",
      });
    }
  );
}
...
function deleteMaterielEtudiant(req, res) {
  ...
  db.query(
    "DELETE FROM materiel_etudiant WHERE id = ?",
    [id],
    (err, result) => {
      if (err)
        return res
